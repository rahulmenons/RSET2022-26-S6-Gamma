...
class busRoute2 extends StatefulWidget {
  const busRoute2({super.key});

  @override
  State<busRoute2> createState() => _BusRouteState();
}

class _BusRouteState extends State<busRoute2> {
  final Location _locationController = Location();
  final Completer<GoogleMapController> _mapController = Completer();
  StreamSubscription<LocationData>? _locationSubscription;

  static const LatLng _pGoogleSrc = LatLng(10.008443, 76.357693);
  static const LatLng _pGoogleDest = LatLng(9.968146, 76.355046);
  LatLng? _busLocation;
      ...
      body: (_busLocation == null || _userLocation == null)
          ...
          : GoogleMap(
              ...
              initialCameraPosition:
                  const CameraPosition(target: _pGoogleSrc, zoom: 11),
              markers: {
  ...
  void _getBusLocationUpdates() {
    _firestore.collection('bus_locations').doc('bus_2').snapshots().listen(
      (snapshot) {
        if (snapshot.exists) {
...
List<LatLng> decodePolyline(String encoded) {
  ...
  bool _alertShown = false; // Flag to track if alert was shown
...
Future<void> _calculateETA() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (etaInMinutes == 2 && !_alertShown) {
        _alertShown = true; // Set flag to true after showing alert

        showDialog(
          context: context,
          builder: (context) => AlertDialog(
            title: const Text("Bus Arrival Alert"),
            content: const Text("The bus is about 2 minutes away!"),
